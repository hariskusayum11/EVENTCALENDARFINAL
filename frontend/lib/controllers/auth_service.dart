...
class AuthService {
 ...
 Future<UserModel?> login(String username, String password) async {
  ...
  // Update user method
  Future<void> updateUser(String id, String name, String lname, String role, String email) async {
    ...
    if (response.statusCode == 200) {
      print('User updated successfully: $name $lname'); // Print success message
    } else {
      final error = jsonDecode(response.body)['message'] ?? 'Failed to update user';
      print('Update user error: $error'); // Print error message
      throw Exception(error);
    }
  }
  ...
  Future<void> deleteUser(String id) async {
    ...
    if (response.statusCode == 200) {
      ...
      print('Delete user error: $error'); // Print error message
      throw Exception(error);
    }
  }
